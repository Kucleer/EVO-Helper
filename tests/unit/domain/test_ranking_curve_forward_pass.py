...
def test_a_reference_that_cannot_stand_up_is_no_reference() -> None:
    """⚠⚠ 参照为负（或与窗口自身水平差出一个数量级）时必须交 `None`。

    负参照让 `abs(score / reference - 1)` 恒大于任何容差 —— **整屏全丢**。而全丢之后
    一个点也进不了历史，下一屏照旧拿同一批坏点去拟合、外推得更远、参照更荒：
    **这是个吸收态**，和 `judge_scores` 里「0 永远不许当基准」是同一类缺陷。

    生产实况（#275 上线后第一趟，20:29–20:30 连着 10 屏）：
    ...
    四个点里只坏一个时，斜率中位数纹丝不动（实测 -10/名，参照 8,920）——那正是
    Theil–Sen 要的效果。**坏两个就够了**：六对斜率里四对变陡，中位数跑到
    -2,510/名，参照 -20,330。而生产上的误读**正好成对出现**（OCR 把同一个值
    重复到下一行，20:28:56 那屏里 9690/9690、3680/3680 都是这样），所以 4 个点的
    窗口对这一类失效本身就是脆的 —— 这一道闸拦的是症状，不是那个脆弱。
    """
    # 名次 1602 / 1603 那两行把 9,0xx 读成了 3,0xx（两行一样的错，生产实拍就长这样）。
    poisoned = [(1600, 9_040.0), (1601, 9_030.0), (1602, 3_020.0), (1603, 3_010.0)]

    reference = curve_reference(poisoned, 1612, require_both_sides=False)

    assert reference is None, f"参照 {reference:,.0f} 站不住却被交了出去 —— 整屏会被全丢"


def test_a_positive_but_absurd_reference_is_also_refused() -> None:
    """⚠️ 闸不能只看正负：坏点落在窗口前半时，参照会正得荒谬。

    同一批坏点换个位置（前两行读小了），斜率中位数变成 **+2,490/名**，
    参照 **+32,170** —— 它是正的，所以光拦负数拦不住；而拿它去判一屏真值
    9,0xx 的读数，结果和负参照一模一样：全丢。
    """
    poisoned = [(1600, 3_040.0), (1601, 3_030.0), (1602, 9_020.0), (1603, 9_010.0)]

    reference = curve_reference(poisoned, 1612, require_both_sides=False)

    assert reference is None, f"参照 {reference:,.0f} 比窗口自身水平高出一个数量级，不应该被交出去"
