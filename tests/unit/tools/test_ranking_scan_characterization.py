...
def _rows(
    scores: Sequence[float | None], *, system: int, first_rank: int
) -> list[RankingRow]:
...
BASELINE_SAID: list[str] = [
    ...
    "军力值不可信，丢掉这几行的分数（坐标保留）[判据 curve/3 · 曲线参照 10430（±3.0%，1/3 行用上了，历史 5 点） · 锚点 10480.0]: [(1, 866, 1044.0, '渲染不出')]",
    "  ⚠️ 与上一屏没有一个共同坐标：重叠可能断了（中间的行没被读过）",
    "  采集第  5滚 读出  3 行 本屏 bot 3 连续空屏 0",
    "军事榜采集完成：真人段走了 800 行（盲滚 700 行 + 检测 0 屏），采集段滚了 5 屏；逐屏写入 12 条，其中末屏可疑 0 条",
...
BASELINE_LIMIT_SAID: list[str] = [
    ...
    "军事榜采集完成：真人段走了 800 行（盲滚 700 行 + 检测 0 屏），采集段滚了 0 屏；逐屏写入 3 条，其中末屏可疑 0 条",
]
