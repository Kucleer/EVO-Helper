...
def test_a_draw_can_be_attacked_again_once_the_revisit_window_has_passed(  # type: ignore[no-untyped-def]
    ...
    ## ⚠️ 别因为 2026-08-17 那条口径又把「平局永久出局」加回来

    用户口径（2026-09-02，逐字）：

        「实际上 平局后，AI 的军力已经大幅度削弱了，他也不会排在前列被扫在范围内了」

    也就是说**游戏机制自己就守住了「不补刀」**：平局之后那个 bot 军力大幅下降，
    掉出军力榜前列，`bot_limit` 那一刀根本扫不到它，读数自然过期、退出候选池。
    再在代码里加一道「平局永久出局」是**用一条会永久生效的判据去解决一个自愈的
    问题**，代价就是 #270 量到的那些：最肥最近的目标被永久锁死。
    """
    from evo_helper.domain.battle_outcome import OUTCOME_DRAW
