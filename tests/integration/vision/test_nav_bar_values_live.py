...
2. `test_every_recipe_in_the_pool_only_ever_drops_digits` —— **池子里每一套配方
   单独看，错法只能是漏字**。`agreed_value` 的安全性整个架在这条性质上；含替换或
   凭空多位错法的配方进池会把汇总规则带崩（实测把 18 套候选全塞进去，读错格从
   0 涨到 9）。
"""
...
def test_every_recipe_in_the_pool_only_ever_drops_digits(reads, truth) -> None:  # type: ignore[no-untyped-def]
    """⚠️ **`agreed_value` 的安全性整个架在这条性质上。**

    池子里每一套配方**单独看**，错法只能是「漏掉了某几位」——不许把某一位读成
    另一个数（替换），也不许凭空多出一位。汇总规则「取最长、其余必须是它漏字后的
    样子」只有在这个前提下才成立：一旦某套配方会臆造，最长的那个就可能是臆造出来的。

    这条也是选配方的判据本身。被剔掉的 `(3,140)` / `(4,140)` 就是栽在这里：
    实拍上它们把 `9` 读成 `93`（凭空多一位）。
    """
