...
| 4 | 控制台整合：去掉定时，保留开始/结束 + 选任务种类 | **未开始** |
...
### 第一步：需求 3 的驱动循环（判定输入已经现成）

还缺的：

1. 扫 1–4 位 → 认「敌对海盗」（`pirate_ui.PIRATE_TITLE_ROI` 已标定）
2. 点「侦察」(886,540) → 绿✓(1156,763) → 简报核对「侦察」→ 出发(1077,815)
3. 等报告（实测约 17 秒）→ 进信箱开最新那封侦察报告 → 慢拖两次 →
   `scout_reports.read_pirate_scout(header, ships, expected_target=那个海盗)`
4. `verdict == ATTACK` 才继续：点「攻击」(1032,540) → 选预设 AAA → 绿✓ →
   简报核对写着「攻击」→ 出发
5. **必须写 intent + dispatch 行**，否则 `/logs` 永远是空的
6. 打完收报告走 `ingest_pirate_report` 那条轻量路径

**尚未标定的只剩「选预设」这一步**：预设条是连续横拖不是分页，一屏只看得见
约两个预设，必须「OCR 定位 → 点」。最右端是「+ 保存当前舰队」，
拖过头点到它会改坏用户的预设——`pirate_ui` 里留了 120px 右边距。

**驱动器要 `allow_actions=True`**（`LiveDriver(allow_actions=True)`），
这是整个仓库里唯一能把舰队送出去的开关，构造点只有一处，别扩散。
