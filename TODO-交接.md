...
1. **配额的「硬信号」没有写入方。** 规格写着 runner 认出超限邮件就封锁当天，
   但全仓没有任何代码写 `mission_tasks.quota_exhausted_until_utc`——认出那封邮件需要
   一个谁都还没见过的画面的 ROI 与关键词。当前只有「数当天派了几发」这一条判据在生效，
   它本身是准的，但**用户手动打过几发的话计数会偏低**，而纠正它的信号是关的。
   补它需要一次实机：撞到超限，把那封邮件截下来。
2. **`BRIEFING_ARRIVAL_ROI` 至今零引用。** 飞行时间只读了时长那一半，所以
   `DispatchBriefing.duration_agrees()` 那道「绝对时间与时长互验」的交叉校验用不上。
   目前唯一的防线是 `MAX_CREDIBLE_FLIGHT = 6 小时`（拦「分」被读成「时」的量级错）。
   那个 ROI 标定之后，正路是切回双来源交叉校验，上界退成第二道防线。
3. **`pirate_loop` 的「回到自己星球地表」坐标仍未量**（见上面第一步，本轮没动）。
