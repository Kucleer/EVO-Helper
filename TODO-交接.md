...
- ~~~~`BRIEFING_ARRIVAL_ROI` 要标定，好切回双来源交叉校验~~ —— **不需要了**。用户
  ...
  是长期方案而不是临时防线。~~ —— ⚠️ **2026-08-19 又翻过来了，而且已经做完了。**
  用户口径（2026-08-18）：「这里飞行时间和预计到达时间 可以进行双保险识别」。
  推翻上面那条的是实机数据：只读时长那一路 24 小时 62 发里读不出 14 发（23%），
  每次白占约 44 分钟航线。ROI 现在拆成
  `BRIEFING_ARRIVAL_DATE_ROI` / `BRIEFING_ARRIVAL_TIME_ROI` 两个（版面是两行，
  而取字一律 `--psm 7` 单行），49 张实拍上 47/47。
  留着上面那段划掉的原话是有用的：它记着「一个来源就够」这个判断是怎么被
  23% 推翻的。
