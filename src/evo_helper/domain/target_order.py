...
⚠️ **它从前的名字是 `top_n`。** 2026-08-23 搬进全局配置那一列时改叫
`window_floor`（`military_attack_config.window_floor`）：那是一列新加的列，没有
存量要迁，而 `top_n` 读起来像「打前 N 名」——那件事 2026-08-18 就已经不发生了。
存量 `params_json` 里的 `top_n` 键**读到就忽略并告警**。
...
#: 用户口径（2026-08-18）：100。
#:
#: ⚠️ **2026-08-23 起它是全局的**（`military_attack_config.window_floor`），
#: 不再按任务（也就是不再按出发点银河系）各配一份。用户口径（2026-08-23）：
#: 「军力攻击的有效期 门限 改为全局设置，不再根据单个星系进行调整」。理由与有效期
#: 那一格相同，写在 `DEFAULT_SCORE_MAX_AGE` 上。存量 `params_json` 里那个 `top_n`
#: 键**读到就忽略并告警**（`application.mission_scheduler._legacy_window_keys`）。
#:
...
#:
#: 分类：**运维旋钮**，在攻击配置页上改（`military_attack_config.window_floor`，
#: 留空才走这里）。调大 = 对「新鲜」要求更苛刻，窗口更常被放弃（于是更常拿旧读数
#: 打，但会告警）；调小 = 更愿意只用窗口内那批，代价是可选目标少、得分最高的那些
#: 可能都在窗口外。没有唯一正确答案。
...
#:
#: ⚠️ **2026-08-23 起它是全局的**（`military_attack_config.score_max_age_hours`），
#: 不再按任务各配一份。用户口径（2026-08-23）：「军力攻击的有效期 门限 改为全局
#: 设置，不再根据单个星系进行调整」。
#:
#: 从前它住在 `mission_tasks.params_json` 里，而任务页 2026-08-22 改版之后一个任务
#: 对应一个出发点银河系——于是它事实上成了「按星系分别配」。**但它约束的不是星系，
#: 是军力榜扫描的节奏**：榜是一趟扫完全宇宙的，一轮扫多久、读数能有多新，与舰队
#: 从哪个银河系出发毫无关系。一个数存成 N 份的代价是改要改 N 处，而漏改一处的症状
#: 是那条链路悄悄用着另一个有效期，页面上一点异常都看不出来。
#:
#: 存量 `params_json` 里那两个旧键（`score_max_age_hours` / `rescan_after_hours`）
#: **读到就忽略并告警**，判据在 `application.mission_scheduler._legacy_window_keys`。
#:
#: 分类：**运维旋钮**，在攻击配置页上改（`military_attack_config.score_max_age_hours`，
#: 留空才走这里）。
DEFAULT_SCORE_MAX_AGE = timedelta(hours=2)

#: 用户能填进去的有效期上界（小时）。
#:
#: 168 小时（一周）的依据：**bot 军力每周一 UTC+0 刷新**，而第 2 步的周期边界
#: （`reading_is_from_this_cycle`）已经把上周期的读数整批挡在外面了。所以有效期
#: 一旦超过一周，它就再也挡不掉任何一条本周期还留得住的读数——**填进去的数不再
#: 有任何后果**，而一个填了却什么都不做的旋钮比没有这个旋钮更坏：用户会以为自己
#: 已经把窗口调宽了。两条判据「都生效、取更严的那个」写在模块头第 2 步。
SCORE_MAX_AGE_MAX_HOURS = 168
...
#: 面板名读不出之后，这个坐标多久之内不再进候选池。**策略，不是游戏规则。**
#:
#: ## 它修的是什么
#:
#: 站到目标星球上，面板上的归属名有时读不出来（`vision.scan_reading.PlanetPanel.
#: display_name is None`）。判据只能说「这不是 bot」，于是这一发不派——而**这件事
#: 一个字都没落库**，下一轮选靶看到的候选池和上一轮一模一样，又把同一个挑出来。
#:
#: 生产库实测（2026-08-20，近 24 小时）：「不是 bot」出现 40 次，**只涉及 3 个
#: 坐标**，而且**没有任何一次是真的读出了名字**——这个判据 100% 是在报识别失败。
#: 这 3 个坐标历史上成功派出 0 次。代价是双份的：
#:
#: - 每撞一次白花 21--44 秒鼠标时间（导航过去 + 读面板 + 复位重试一遍）；
#: - 更贵的是**整轮空手而归**（65 轮里 16 轮，25%），于是 `waiting_for_a_line`
#:   把那颗球压到「下一条航线空出」为止——实测一次压了 117 分钟。
#:
#: ## 6 小时这个数怎么来的
#:
#: 三条边把它夹在这里：
#:
#: - **下界由「重新挑中它」的节奏定。** 那个 24 次/24 小时的坐标约每小时被挑中
#:   一次；排除窗口若不明显大于 1 小时，等于没排。而空轮把航线压住约 1--2 小时，
#:   窗口短于 2 小时就会在那次压制刚解除时原样再撞一发。
#: - **上界由「万一是偶发」定。** 这 3 个坐标是军力 39,030 / 20,960 / 20,630 的
#:   高价值目标，而**面板名为什么读不出，根因至今不明**（画面没加载完？窗口失焦？
#:   还是这个坐标就是读不出？）。排一整天等于凭一次读不出就把一个可能很好的目标
#:   丢掉一天，而我们并没有证据支持那个结论。
#: - **6 小时把观测到的浪费砍掉约四分之三**（40 次/24 小时 → 约 12 次），同时每个
#:   坐标一天仍有 4 次机会自己证明「我其实读得出来」。
#:
#: ⚠️ **排除是有尽头的，故意的。** 见 `storage.models.BotTargetRow.
#: unreadable_attempts`：永久拉黑靠的是一个我们解释不了的判据，而「连续失败第 N
#: 次」写进库、写进日志，让用户自己看得出哪些是无可救药的。
#:
#: 这是「没配置时」的默认值，页面上有一个框
#: （`military_attack_config.unreadable_exclusion_hours`），留空才走这里。
DEFAULT_UNREADABLE_EXCLUSION = timedelta(hours=6)

#: 用户能填进去的「面板名读不出」排除时长上界（小时）。
#:
#: 24 小时的依据和 `PROTECTION_EXCLUSION_MAX_HOURS` 同源：越过一天就跨到
#: `bot_revisit_hours` 的默认值那一侧，两个旋钮开始争同一件事，「这个坐标为什么
#: 进不了候选池」变成一道多选一的谜题。而且这一条比保护期更该守住上界——保护期
#: 至少是个**读得懂的事实**（弹窗明说了），而「读不出」是个**我们还没查清的现象**，
#: 按它把一个高军力目标锁掉超过一天，赌的是一个还没有证据的结论。
UNREADABLE_EXCLUSION_MAX_HOURS = 24
