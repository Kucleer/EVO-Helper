...
class TaskStatus(Enum):
    ...
    #: 一是「从没上过军力榜」，二是 2026-08-19 加的「读数全属于上一个周期」——
    #: bot 军力每周一 UTC+0 刷新，刷新那一刻全库读数同时作废
    #: （`domain.target_order` 模块头第 2 步）。周一凌晨整池都是第二种。
    #: 不为第二种另开一档，是因为页面上多一档只会多一个要解释的词，而用户该做的
    #: 动作一模一样；**要分清是哪一种就去看日志**——
    #: `application.mission_scheduler._log_a_pool_stalled_on_the_cycle_boundary`
    #: 专为此存在，它把本周期起点、作废了多少条、还剩多少都写出来。
    #:
    #: ⚠️ **措辞 2026-08-18 换过一次，从「军力数据已过期」改成「未采集」。**
    #: 那一版里超期的目标会被整批滤掉，所以「一个能打的都没有」确实可能是超期
    #: 造成的。现在超期**不会**让这一档亮起来：读数超期时选靶走的是「放弃窗口」
    #: 那条路（见 `WIDENED_SCORE_WINDOW`），照样打得出去。于是这个状态**只剩
    #: 一个成因**：候选全都从没上过军力榜。仍然写「已过期」的话，用户会去把
    #: 有效期调长——而调长解决不了「一条读数都没有」。
    #:
    #: ⚠️ **判据（`scores_are_missing`）数的是第 2 步的余量**（有本周期读数的候选数），
    #: 而窗口筛选是第 3 步的事——第 3 步筛空了不会让这个数变 0。两者说的是两件事：
    #: 这一档是「本周期一条读数都没有」，`WIDENED_SCORE_WINDOW` 是「有，只是都旧了」。
    #:
    #: ⚠️ **「读数属于上一个周期」落在这一档，不落在 `WIDENED_SCORE_WINDOW`。**
    #: 这是判据放在第 2 步（而不是在放宽那里补一个 `if`）换来的：上周期的读数在
    #: 第 2 步就出局，放宽也捞不回来，所以周一凌晨页面说的是「军力数据未采集」，
    #: 而不是一句听起来一切正常的「已放宽窗口」。
    #:
    ...
    #: 仅军力优先：**有活干，但这一轮是放宽了有效期窗口才凑出来的**——用到了
    #: 窗口外的旧读数。
    #:
    #: ⚠️ **它替代的是「待命」，不是某个「不动」的原因。** 放宽的时候任务照跑、
    #: 照派遣，所以它必须排在 `has_work` 的那一支里；放到下面那堆「为什么不跑」
    #: 里去的话，它只会在任务本来就不动时才显示，而那正是它最没用的时候。
    #:
    #: ⚠️ **它不排在「等航线」「冷却中」前面。** 那两档回答的是「为什么不动」，
    #: 而这一档回答的是「动是动了，用的是什么数据」。同时成立时先说「为什么不动」
    #: ——用户对着一个不动的任务，最想知道的是它什么时候动。
    #:
    #: 存在的理由是用户 2026-08-18 的原话：「今晚这件事的真正问题不是『用了旧数据』，
    #: 而是**用了旧数据却没人告诉你**——你是从攻击日志里一条一条对出来的」。
    #: 日志那一半在 `application.mission_scheduler._warn_about_a_widened_window`。
    WIDENED_SCORE_WINDOW = "军力读数已放宽窗口"
...
class TaskFacts:
    ...
    **星球**算，海盗每天 32 次按**账号**算。把它们摊平在一个扁平的结构里，迟早
    ...
    #: 仅军力优先：`targets_remaining` 归零**是不是因为候选一个本周期的军力读数
    #: 都没有**。
    #:
    ...
    #:
    #: ⚠️ **2026-08-19 起「周一刷新、上周期读数整批作废」也走这一档**，
    #: 理由与措辞在 `TaskStatus.MISSING_MILITARY_SCORES` 上。那一档同样必须盖住
    #: 「已完成」——周一凌晨读数全废时页面写「已完成」，正是这条判据要修的形状。
    #:
    #: 2026-08-18 之前它叫 `scores_are_stale`、成因是「分数全部超期」。超期不再
    #: 让候选归零（读数都旧了就放弃窗口、照打，见 `domain.target_order`），
    #: 那个成因随之消失，剩下的唯一成因就是「从没上过军力榜」——名字跟着改，
    #: 否则页面会指着一个不存在的原因。
    #:
    ...
    #:
    #: ⚠️ **它和 `scores_are_missing` 说的是两件事，别合并。** 那一个是
    #: 「一条读数都没采到」（第 2 步余量为 0），这一个是「采到了，只是都旧了」
    #: （第 3 步筛不够）。合成一个布尔的话，页面只能二选一地说错其中一件。
    #:
    #: ⚠️ **它不参与任何调度判据**，只决定页面显示成哪一档。放宽的时候任务照跑
    #: ——那正是这次改动要的：**宁可用旧数据打，也不能不吭声地用旧数据打**。
    scores_window_widened: bool = False
...
def free_lines_for(task: TaskSnapshot, *, inflight_from_origin: int, reserved_lines: int) -> int:
    ...
    **`inflight_from_origin` 必须只数同一颗出发星球上的在飞派遣。** 游戏的航线
    ...
    「两者均需要约束」），由 `account_free_lines` 算，两者取小——**只在用户填了
    上限时才有**。这里刻意不合成一个函数：两个数的作用域不同（一颗星球 /
    一个账号），合起来之后调用方就分不清自己传的在飞数该数谁了，而数错了不报错。
...
def account_free_lines(
    *, account_limit: int | None, inflight_total: int, reserved_lines: int
) -> int | None:
    """**全账号**此刻还剩几条航线。与出发星球无关。`None` = 这道闸不生效。

    用户口径（2026-08-18）：「我的总航线数是所有星球共享的，在启动加成道具情况下
    最高是到 9 条」「星球的航线是我来配置的，我配置时已经手动确认了不会超过总航线
    数，两者均需要约束」。

    ## ⚠️ 为什么 `account_limit` 可以是 `None`，而且**故意没有代码默认值**

    用户口径（2026-08-18）：「账号的默认权限不应在代码中进行配置，直接用航线限制
    就可以了，因为实际通过科技升级，使用道具，人为占用，都会影响到留给你的航线
    数量」。也就是说**真实可用航线是浮动的**——科技升到哪一级、这会儿开没开加成
    道具、用户自己手上占着几条，每一样都会变。代码里写死任何一个数都是错的，
    9 是错的，6 也是错的：写死之后助手会拿一个凭空的数去卡用户真实的处境，
    而它凭什么是那个数，谁也说不出来。

    **所以 `None` 的含义是「不额外限制账号总数」，不是「用某个默认值」。**
    这一档仍然安全，因为每颗星球那道预算照常生效，而用户口径同一天还有一句：
    「星球的航线是我来配置的，我配置时已经手动确认了不会超过总航线数」——
    每星预算之和不超真实总量这件事由他保证。

    ⚠️ **下一个人最容易做的错事就是「顺手补一个默认值」**（看起来像是漏了）。
    补上之后症状是静默的：助手按一个猜出来的上限少派几发，页面上一切正常。
    要改这个数请改**配置**，不要改代码。

    ⚠️ **也不许回落到 `scheduler_config.fleet_line_limit`。** 那一列现在的语义是
    「任务没填航线数时用几条」，借过来用等于让一个数同时表达两件事，造出第二份
    真相。
    ...
    出去的舰队占的正是账号总数里的位子——用户那句「**人为占用**也会影响留给你的
    航线数量」说的就是它。
    """
...
def status_of(
    ...
    **不新写判据。** 「有没有活干」一律问 `has_work`；只有它说「没有」时才
    ...
    if has_work(task, facts, restart_cooldown=restart_cooldown):
        # ⚠️ **「放宽了窗口」只替换「待命」这一档，不替换别的。** 放宽的时候任务
        # 照跑、照派遣，所以它属于「有活干」这一支；挪到下面那堆「为什么不跑」
        # 里去的话，它只会在任务本来就不动时才显示——而那正是它最没用的时候。
        #
        # 为什么值得占掉「待命」：用户 2026-08-18 的原话是「真正的问题不是用了
        # 旧数据，而是用了旧数据却没人告诉你」。「待命」是一句完全正常的话，
        # 顶着它把三天前的读数打出去，正是这次要修的那个形状。
        if task.kind is MissionKind.BOT and facts.of(task).scores_window_widened:
            return TaskStatus.WIDENED_SCORE_WINDOW
