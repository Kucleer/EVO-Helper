...
class TaskStatus(Enum):
    ...
    #:
    #: ⚠️ **判据（`scores_are_missing`）2026-08-18 二次改造之后仍然成立，没有动。**
    #: 它数的是第 2 步的余量（有读数的候选数），而窗口筛选是第 3 步的事——
    #: 第 3 步筛空了不会让这个数变 0。两者说的是两件事：这一档是「一条读数都没
    #: 采到」，`WIDENED_SCORE_WINDOW` 是「采到了，只是都旧了」。
    #:
...
class TaskFacts:
    ...
    **星球**算，海盗每天 32 次按**账号**算。把它们摊平在一个扁平的结构里，迟早
    ...
    #: 仅军力优先：**这一轮是放宽了有效期窗口才凑出来的**——窗口内不够军力截断
    #: 要的那么多个，于是在全部有读数的目标里按军力截断，用到了窗口外的旧读数。
    #:
