...
class MilitaryAttackConfigOut(BaseModel):
    ...
    #: 军力榜采集开榜后先用滚轮连拨几**行**。**`None` = 留空 = 按实测自动标定。**
    #:
    #: 这里只认「是不是整数」；范围由 `MissionScheduler.validate_blind_scroll_rows`
    #: 判，和调度器组命令行时用的是同一把尺子——两边分家的结果是页面收下了、
    #: 实机跑起来不是那个数。
    #:
    #: ⚠️ **`0` 是合法取值**（「一格都不拨」，最保守的那一头），不是「留空」。
    #:
    #: ⚠️ **这一项显式 `strict=True`，下面那些旋钮没有。** 非严格模式下 `bool` 会被
    #: 当成 `int` 收下（`true` 存成 1 行），而「拨一格」和用户想说的话没有任何关系。
    #: `_blind_scroll_rows` 那边本来就把 `bool` 单独排掉了，只是 pydantic 在它之前
    #: 就把 `true` 换成了 1、调度器再也看不见那是个布尔——这里补的是那一段。
