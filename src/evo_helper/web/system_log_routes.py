...
def _parse_moment(value: str | None) -> datetime | None:
    ...
    浏览器交上来的是不带时区的本地挂钟（`2026-08-16T09:30`），这里按 **UTC** 读。
    解析不了就当没填：一页 422 的 JSON 读起来就是「控制台坏了」。

    ⚠️ **它和页面上那一列现在不是同一个时区了。** 时间列已经改成按 UTC+8 显示
    （用户口径 2026-08-17：页面上的时刻一律 UTC+8），而这两个框还是 UTC——
    差 8 小时，页面上用一句话明写了出来（见 `system_log.html` 的筛选说明）。

    没有跟着改，是因为这个函数**同时**是 `GET /api/system-log` 的取值口径：
    换成 UTC+8 就是悄悄改了接口的查询语义，任何存着的链接、任何调它的脚本都会
    在没有报错的情况下查到另一个八小时。要改，得连接口一起改，并且是一次
    有意的接口变更。
    """
    text = _blank(value)
    if text is None:
