...
class AttackLogView:
    ...
    #: 回收发的实收读回来了没有（回收报告邮件里那三个资源数）。
    #:
    #: ⚠️ **邮件链路上线之前恒为 False**，页面因此恒显示「待回收」。
    #: 那是个**真实的待办**（信躺在信箱里没人读），不是「待战报」那种
    #: 等一个永远不会来的东西——两者在页面上必须是两个词。
    recycle_haul_back: bool = False
    ...
    #: **派这一发时看到的目标军力**，以及那个读数是什么时候读到的。
    #:
    #: ⚠️ **两个都取自 `attack_intents` 上的快照列，绝不去 join `bot_targets`。**
    #: 那张表存的是当前值，每采一次军力榜就整行覆盖（生产实测 2026-08-18：同一批
    #: 目标一天内从 31,756 刷到 2,616）。现取答的是「它现在多强」，而这一列要答的
    #: 是「当时凭什么打它」——事后 join 会让日志随着扫描不断改口，且毫无痕迹。
    #:
    #: ⚠️ **None 不能显示成 0。** 海盗位在 `bot_targets` 里没有行，还没上过榜的 bot
    #: 有行但没读数，两种都是 None，页面显示「—」；而被打空的 bot 军力真的是 0。
    #: 「没读数」和「读数是 0」在这一页上必须分得开。
    #:
    #: ⚠️ **快照列为 NULL 的历史行照实显示「—」，不许拿现值回填。** 这一列是
    #: 2026-08-18 才开始记的，之前的意图没有这个观测——补一个看起来合理的数进去，
    #: 就再也分不出哪几行是真的读到过。
    target_military_score: float | None = None
    target_military_score_at_utc: datetime | None = None
    #: 上面那个分数**是不是插出来的**（同一份快照的第三列）。
    #:
    #: 为真时的含义不是「大概齐」：这一行从榜上读到的分数破坏了降序、被判为不可信
    #: 丢掉了，显示的数是上下两个好邻居插出来的中点。规模不小——2026-08-18 生产库
    #: 3225 个有读数的 bot 里估算 365 个（11.3%），不标出来就是每九个里有一个把
    #: 插值当实读展示。
    #:
    #: ⚠️ **三档，不是两档。** `True` 标「(估算)」、`False` 什么都不标、`None` 也
    #: 什么都不标——但两种「不标」的含义完全不同：`False` 是「这个数是实读的」，
    #: `None` 是「2026-08-18 之前派的，当时没记这件事」。所以视图层保留 `None`，
    #: 不许在这里折成 `False`：那等于让历史行冒充实读。
    target_military_score_estimated: bool | None = None
    #: 那份战报**有没有存下截图**。
    #:
...
class AttackLogOptions:
    ...
    #: `mission_task_origins` 里配着 2 颗，而 `attack_intents` 里真正出现过的出发点
    #: 是 3 个，其中最大的那一个（占全部意图的 72%）**从来没进过配置表**。
    #: 只取前者的话，占日志七成的那些记录根本筛不出来，而页面上看不出少了它，
    #: 用户只会觉得「筛选坏了」。
    #:
...
class MissionTaskView:
    ...
    `detail` 是随行的事实（今日 12/32、还剩 37 个未完成）。
...
class MilitaryAttackConfigView:
    ...
    #: 读不读回收报告邮件。⚠️ 默认关是有意的：这是加功能不是修 bug。
    recycle_mail_enabled: bool = False
