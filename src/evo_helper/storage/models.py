...
class MilitaryAttackConfigRow(Base):
    ...
    #: 军力读数「算不算新」的门槛，也就是选靶第 3 步那扇**窗口**的宽度（小时）。
    #: **空 = 用 `domain.target_order.DEFAULT_SCORE_MAX_AGE` 的 2 小时。**
    #:
    #: ⚠️ **2026-08-23 从 `mission_tasks.params_json` 搬到这张全局表。** 用户口径
    #: （2026-08-23）：「军力攻击的有效期 门限 改为全局设置，不再根据单个星系进行
    #: 调整」。任务页 2026-08-22 改版之后一个任务对应一个出发点银河系，于是这一格
    #: 事实上成了「按星系分别配」——而**它约束的不是星系，是军力榜扫描的节奏**：
    #: 榜是一趟扫完全宇宙的，扫得多快、读数多新，与从哪个银河系出发毫无关系。
    #: 一个数按星系存了三份，改起来要改三处，而漏改一处的症状是那条链路悄悄用着
    #: 另一个有效期，页面上看不出来。
    #:
    #: ⚠️ **是浮点，不是整数。** 页面上一直允许填 1.5 小时（步长 0.5），
    #: 存成整数会把用户配的值悄悄取整。
    #:
    #: 可空、不给 `server_default`：NULL = 「跟着代码默认走」，同本表其余旋钮。
    score_max_age_hours: Mapped[float | None] = mapped_column(Float, nullable=True, default=None)

    #: 选靶第 3 步的**窗口门限**：窗口内至少要有这么多个目标，这一轮才肯只用窗口
    #: 内的。**空 = 用 `domain.target_order.WINDOW_POOL_FLOOR` 的 100。**
    #:
    #: ⚠️ **它不决定打谁。** 2026-08-18 起第 4 步按 `军力 ÷ 往返小时` 排序，
    #: 军力硬截断取消，这个数只剩「这一轮肯不肯只信新数据」这一个身份。整段理由在
    #: `domain.target_order.WINDOW_POOL_FLOOR`。
    #:
    #: ⚠️ **列名是 `window_floor`，不是从前那个 `top_n`。** 旧名字留在
    #: `mission_tasks.params_json` 里是因为改名要迁移一批存量 JSON 键；而这是一列
    #: 新加的列，没有存量，没有理由把一个 2026-08-18 就已经名不副实的名字再刻一遍
    #: ——`top_n` 读起来像「打前 N 名」，而它早就不是了。
    #:
    #: 它和有效期是**同一道判据的两半**（「窗口多宽」＋「窗口内够不够用」），
    #: 所以一起搬成全局：一半全局一半按星系，等于让同一道判据的两半各说一套。
    window_floor: Mapped[int | None] = mapped_column(Integer, nullable=True, default=None)

    # ⚠️ **这里曾经有一个 `military_time_pool`（「军力时间池」），2026-08-18 删掉了。**
    # 迁移 `b1d9e47f2a03`。它的存在本身就是一个错误设计的产物：那一版选靶第 3 步是
    # 「按读数时间取前 N 个」，而军力榜从强到弱扫，「读数最新」系统性地等价于
    # 「军力最弱」——那个旋钮调的其实是「往最弱的那一头走多远」。现在第 3 步换成
    # 按有效期**划线**（上面那一列 `score_max_age_hours`），不再有「取前几个」这件
    # 事，所以这一列没有对应的语义可留。整段理由在 `domain.target_order` 模块头第 3 步。
    #
    # **别把它加回来。** 想控制「用多新的数据」就调那个有效期。
