...
class PirateLoop:
    ...
    def _sub_tab_matches(self, *, fleet: bool, name: str, quiet: bool) -> bool:
        ...
        #     问「关掉了？」那 188 次：舰队类 0/6 行，100%
        #
        # ⚠️⚠️⚠️ **但那 188 里有 172 条来自 #339 自杀的那 57 分钟**（2026-09-14
        # 第三轮复核催出来的：我原先的 SQL 没限定窗口）。那段时间代码正在每轮自杀、
        # 屏幕本来就读不出来 —— 拿它当「筛选关着时的正常表现」是不成立的。
        # **正常运行期的样本只有 16 条。**
        #
        # 所以这个阈值的真实依据是：**28 条 vs 16 条**，两侧仍然不重叠
        # （4~6 对 0），但**证据比「188」听上去弱得多**。别再引用 188。
        #
