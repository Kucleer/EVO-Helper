...
class PirateLoop:
    ...
    def _sub_tab_matches(self, *, fleet: bool, name: str, quiet: bool) -> bool:
        ...
        #             每一轮都在开工那步自杀。**全线停摆 3.5 小时，一发都派不出去。**
        #
        # 实测事实只有一条，两次都是没照它办：
        #
        ...
        # 开着时 100% 认得出，关着时 188/188 不误判。
        #
        # ⚠️ 我在这一行上已经错过三次，全是没查这组数就动它：
        #   09-13 夜  改成「没看见舰队类就算关掉」→ 静默饿死 9 小时
        #   09-14 午  改成「必须看见非舰队类」   → 每轮自杀、停摆 3.5 小时
        #   09-14 复核 差点改成「整屏全中」      → 会漏掉 54% 的卡住筛选（5/6 与 4/6 那两档）
        #
        # `other == 0` 留着：它只会让「判成开着」更难，挡的是「没筛过却碰巧全是
        # 舰队类」那种反手把筛选打开的事（实测 0/188，但留着不花钱）。
