...
class PirateLoop:
    ...
    def _sub_tab_matches(self, *, fleet: bool, name: str, quiet: bool) -> bool:
        ...
        # ⚠️ 判据是**整屏全中**，不是「有舰队类且没有非舰队类」。
        # 后者在**没筛过的列表上也会成立** —— 舰队类占信箱三分之二，那一屏碰巧
        # 全是舰队类、而攻击报告的主题又读不出（常态），它就成立了。那时代码
        # 会以为筛选开着、点一下，**反而把筛选打开**，退回赌奇偶。
        # 实测事实是「开着筛选时 6/6 全中」，所以照它写：一行 unknown 都不许有。
        on = bool(kinds) and here == len(kinds)
    ...
    def _scan_for_reconcile(
        ...
        #
        # ⚠️⚠️⚠️ **但「查不出来」不许判死这一轮。** 2026-09-14 上午我正是这么写的
        # （关不掉就抛），而「关不掉」在读不出主题时**必然成立** —— 于是每一轮都在
        # 开工这步自杀，攻击、扫描、全部任务一起停摆 3.5 小时，一发都派不出去。
        #
        # 所以放弃这一趟的条件是**正面认出筛选还开着、而且关不掉**，
        # 不是「确认不了它关着」。认不出开着 ⇒ 它没开着 ⇒ 照常翻。
        # 这一条成立是因为舰队类主题读得准（实测 6/6），「开着」这个方向
        # 拿得到正面证据 —— 整段理由在 `_sub_tab_matches`。
        if self._fleet_filter_looks_on() and not self._select_mail_sub_tab(fleet=False):
            say("  舰队筛选开着又关不掉；这一趟不翻信箱（在筛过的列表上翻等于静默读空）")
            self._dump_frame("reconcile-fleet-filter-stuck", PANEL_TITLE_ROI)
            raise RuntimeError("舰队筛选开着又关不掉，拒绝在筛过的列表上翻战报")
