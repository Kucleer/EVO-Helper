...
#: 「报告」底下第二排的**战斗**筛选（client 坐标，实拍 2026-09-15
#: `dump-mail-list-empty-014934.png` 量出：战斗跨 x713~830，中心 772）。
#:
#: ⚠️⚠️⚠️ **`#336` 曾经把这个按钮判成「不响应」并删掉了它，那个结论是错的。**
#: 当时的代码本来就是点「战斗」切回去的，只是坐标抄错（897 是**侦察**的 x）；
#: 改对成 772 之后那一次没生效，于是我把整条路否掉，改成「再点一次舰队把它取消」。
#:
#: 2026-09-15 实拍推翻了它：**取消舰队之后不是「显示全部」，是「什么都不显示」** ——
#: 第二排一个筛选都没选中时，列表正中写着「没有符合当前筛选条件的邮件。」，
#: 而角标明明写着战斗 11 封、舰队 99+ 封。
#:
#: 代价：攻击战报整晚读不到（09-14 06:37 → 09-15 02:00，一份都没有）。
#:
#: ⚠️ 名字要绕开 `human_input.ACTING_WORDS`（只读进程点带「攻击/派遣/取消…」的标签
#: 会当场崩）。「战斗」一个词都不沾，但改名字之前先回去看那张表。
MAIL_BATTLE_SUB_TAB = (772, 218)
...
#: 点完标签之后，判据最多**再看几次**（每次之间等 `MAIL_SUB_TAB_SETTLE_WAIT_S`）。
#:
#: ⚠️⚠️ 这一档是 2026-09-15 实拍逼出来的：点「战斗」之后列表要过几秒才换掉，
#: 而判据只看一次就判失败、接着又点一次（很可能把刚切好的点回去）。
#: 失败时存的现场图上列表第一行已经是「攻击报告」了 —— **那一下本来是成功的**。
#:
#: ⚠️ `#336` 当年「战斗按钮不响应」的结论多半也是这个时序假象，
#: 而那个错误结论让「切回战报」走错了两天。
#: ⚠️ **4 → 8（2026-09-15 03:40，实测加大）。** 8 秒不够：
#: 统计那一晚所有「以舰队类为主、却混进一两行非舰队类」的屏，
#: 污染行**集中在底部** —— 第 4 行 5 次、第 5 行 5 次，第 1/3 行合计才 3 次。
#: 也就是说列表是**自上而下重绘的，底部那几行更新得最晚**，
#: 而判据要求整屏干净，于是在底部还没跟上的那几秒里必然判失败。
#:
#: 对照：整屏一次性更新时，「点击 → 认出」实测只要 **1.9 秒** ——
#: 所以多等的代价只落在本来就要失败的那些趟上。
#:
...
class PirateLoop:
    ...
    def _select_mail_sub_tab(self, *, fleet: bool) -> bool:
        ...
        """
        # ⚠️⚠️ **两个方向点的是不同的按钮，这是 2026-09-15 实拍改过来的。**
        #
        # 先前两个方向共用 `MAIL_FLEET_SUB_TAB`，把它当开关：切回去 = 再点一次把舰队取消。
        # 实拍推翻了那个模型 —— **取消之后不是「显示全部」，是「什么都不显示」**：
        # 第二排一个筛选都没选中时列表正中写着「没有符合当前筛选条件的邮件。」，
        # 而角标写着战斗 11 封、舰队 99+ 封。整段经过在 `MAIL_BATTLE_SUB_TAB` 上。
        #
        # 所以：要舰队就点舰队，要战报就点**战斗**。
        ...
        # ⚠️ 名字要和**实际点的按钮**一致：先前两个方向共用舰队坐标，所以叫
        # 「舰队筛选关」；现在 `fleet=False` 点的是**战斗**，名字不改日志就会骗人。
        ...
        for attempt in range(MAIL_SUB_TAB_TRIES):
            ...
            if not self._on_mail_list():
                ...
                continue
            # ⚠️⚠️ **点完之后要多看几次，不能只看一次就判失败。**
            #
            # 2026-09-15 02:31 实拍（`dump-mail-sub-tab-战斗-unconfirmed-023122.png`）：
            # 点「战斗」之后判据连着三次读到的都还是**舰队类**，于是判定失败、
            # 又点了一次（很可能把刚切好的又点回去）；而失败时存下来的那张现场图上，
            # 列表第一行赫然是**「攻击报告」**、角标 15 —— **那一下其实成功了**，
            # 只是列表刷新比判据慢。
            #
            # ⚠️ `#336` 当年「战斗那个按钮不响应」的结论，多半也是同一个时序假象 ——
            # 而那个错误结论让「切回战报」整整走错了两天。
            #
            # 多等的代价是每趟最多几秒；判错的代价是攻击战报整晚读不到。
            for _ in range(MAIL_SUB_TAB_SETTLE_READS):
                if self._sub_tab_matches(fleet=fleet, name=name, quiet=True):
                    say(f"  二级标签在「{name}」上（等列表刷新后认出来的）")
                    return True
                self._driver.wait(MAIL_SUB_TAB_SETTLE_WAIT_S)
            # 最后再大声读一次，把「到底读到了什么」留在日志里。
    ...
    def _fleet_filter_looks_on(self) -> bool:
        """**正面**认出舰队筛选还开着吗。认不出就当它没开着。

        ⚠️ 这是这条链路上唯一拿得到正面证据的方向：舰队类主题读得准（实测 6/6），
    ...
    def _sub_tab_matches(self, *, fleet: bool, name: str, quiet: bool) -> bool:
        ...
        #
        #   09-13 夜：把「关掉」这一侧放松成「没看见舰队类就算数」——
        #             筛选卡在舰队上没人发现，攻击战报断流 9 小时、37 发无战报。
        #   09-14 午：把它改严成「必须正面看见非舰队类」——
        #             而非舰队类的主题**本来就读不出**，于是永远确认不了，
        #             每一轮都在开工那步自杀。**全线停摆 57 分钟，一发都派不出去。**
        #
        # 实测事实只有一条，两次都是没照它办：
        #
        #     **舰队类（舰队返回 / 回收报告）的主题读得准（6/6）；
        #       非舰队类（攻击报告…）读不准，整屏 unknown 是常态。**
        #
        # 所以：**只在「筛选开着」这个方向上判，那是唯一拿得到正面证据的方向。**
        # 「关掉了」不去正面确认 —— 它等价于「认不出筛选开着」，而那正是
        # `_fleet_filter_looks_on()` 返回 False 的意思。
        other = sum(
            1
            for kind in kinds
            if kind is not None
        ...
        # ⚠️⚠️⚠️ **这个阈值是照下面两组日志计数**挑**出来的，别再凭印象收紧或放宽。**
        #
        # ⚠️ 别把它说成「216 次生产实测」—— 那是个作废的口径（见下）：
        # 那 216 里混着 172 条故障期日志，而且这两组都不是「实测筛选状态」。
        #
        # 2026-09-14 复核时把库里所有这两类日志拉出来数了一遍：
        #
        #     问「开着？」那 28 次：舰队类 6/6 行 46% · 5/6 行 42% · 4/6 行 10%
        #                           —— 永远 ≥ 4，一次 0 都没有（全部来自正常运行期）
        #     问「关掉了？」那 **16** 行（**正常运行期**）：舰队类 0/6 行
        #
        # ⚠️ 我原先在这里写的是「188 次」。那个数里有 **172 条来自 #339 自杀的
        # 那 57 分钟**（第三轮复核催出来的：原先的 SQL 没限定窗口）——
        # 那段时间代码正在每轮自杀、屏幕本来就读不出来，不能算「正常表现」。
        # **别再引用 188。**
        #
        # ⚠️⚠️ **而且这两组数都不是筛选真实状态的样本**（第四轮复核指出）：
        # 分组依据是日志里判据自己的措辞，没有独立状态标签，也没有按页面去重。
        # 它们能说的仅止于：
        #
        #     问「开着？」的那 28 行，舰队类计数落在 4~6；
        #     问「关掉了？」的那 16 行，落在 0。
        #
        # **`here > 0` 作为判据的识别效力，这批数据回答不了，至今未独立验证。**
        # 这个阈值是照这两组计数**挑**的，不是被它们**证明**的。
        #
        # 两侧之间是个大空档，所以 `here > 0` 就把它们分干净了：
        # ⚠️ **这两组数是「日志里的 OCR 计数分布」，不是筛选的真实状态**
        # （2026-09-14 复核指出：状态标签取自判据自己的成功文案，属**循环分类**）。
        # 它支持「`here > 0` 落在两组分布之间的空档里」这个阈值选择，
        # **不能**据此宣称「100% 认得出」或「0 误判」—— 那要独立标注才能讲。
        #
        # ⚠️ 我在这一行上已经错过三次，全是没查这组数就动它：
        #   09-13 夜  改成「没看见舰队类就算关掉」→ 静默饿死 9 小时
        #   09-14 午  改成「必须看见非舰队类」   → 每轮自杀、停摆 57 分钟
        #   09-14 复核 差点改成「整屏全中」      → 它严格强于 `here > 0`，只可能认出更少：
        #                                        那 28 行里 54% 不是满屏（5/6、4/6 两档）。
        #                                        ⚠️ 这是**日志计数比例**，不是「漏检率」;
        #                                        这批样本没有独立状态标签。
        #
        # ⚠️ `other == 0` 留着，但**别把它当成挡住「没筛过却碰巧全是舰队类」的理由**
        # —— 那种屏幕的 `other` 恰恰就是 0，这一项挡不住它（2026-09-14 复核指出，
        # 我原来的注释在这一点上逻辑不成立）。
        # ⚠️ 目前**没有任何运行时机制**挡住它 —— 只有一组**观察**：
        ...
        #
        # 上面那道空列表闸已经把「没有符合当前筛选条件的邮件」挡在前面并 return 了，
        # 所以走到这里就说明**这一屏有东西**。而战斗标签里混着攻击报告等等，
        # 它们的主题读不准（实测 87% 读不出），拿它们要正面证据等于永远判不成立
        # —— 那正是 2026-09-14 中午「每轮自杀」的成因。
        #
        # ⚠️ 剩下要排除的只有「点完还停在舰队上」，那一档 `on` 为真，照样拦得住。
        ...
        **未读**邮件 = 它会一直排在别人后面，直到掉出扫描下限（默认 6 小时），
        ...
        if fleet_sub_tab and not self._select_mail_sub_tab(fleet=True):
            return MailScan(
                ...
                # ⚠️⚠️ **`aborted=True` 不能漏。** 它的文档里逐字写着这一档：
                # 「半路出事（丢了邮件列表重进不回、**切不回二级标签**）」。
                # 漏掉的后果是 `tally.swept` 判成 True、对账时刻照写 ——
                # **一趟什么都没扫的行程被记成「翻完了」**，而那正是 09-13 夜
                # 那次事故的同一个病：失败的一趟被记成成功的一趟。
                aborted=True,
                cut_short="切不到「舰队」二级标签，这一趟不读回收报告",
            )
        # ⚠️⚠️ **要没筛过的列表的那几趟，自己保证筛选是关的。**
        #
        # 2026-09-13 夜的事故：回收那一趟把信箱留在舰队筛选上，读战报那一趟照常
        # 开工、整屏按主题全拒 —— 日志上和「信箱里没有战报」一模一样，9 小时没人
        # 发现、37 发攻击没有战报。那条「读完切回去」的还原义务只写在回收那一侧，
        # 它一失手，代价全落在这一侧而这一侧毫不知情。
        #
        # ⚠️ **位置很要紧：必须在 `_enter_mailbox()` 之后。** 09-14 我第一版把它写在
        # `_scan_for_reconcile` 开头，那时信箱还没开，探测读的是恒星系视图、
        # 一行都读不到 —— 每轮白花一次 OCR，一点保护都没有。
        #
        # ⚠️⚠️ **认不出就放行，不许判死。** 同一天我还写过「关不掉就抛异常」，
        # 而「关不掉」在主题读不出时必然成立 —— 每一轮都在开工那步自杀，
        ...
        if not fleet_sub_tab and self._fleet_filter_looks_on():
            if not self._select_mail_sub_tab(fleet=False):
                ...
                # 提前返回都**绕过了函数末尾的 `_close_mail()`** —— 信箱面板留在屏上，
                # 后续任务是否自行收尾没有验过。不要想当然认为它会导致误点击，
                # 也不要想当然认为没事。
                return MailScan(
                    unread_budget=max_unread_opens,
                    # ⚠️⚠️ 同上：**这一档必须 `aborted=True`**，否则对账时刻照推进，
                    # 后面的轮次会因为冷却而跳过，而这一趟其实一封都没看过。
                    aborted=True,
