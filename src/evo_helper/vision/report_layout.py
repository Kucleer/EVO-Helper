...
#: 读「回收船」那一格时，在标定框上下各试哪几档偏移（像素，viewport 空间）。
#:
#: ⚠️ **这不是「调框」，是出候选。** 「哪个框更好」在这个仓库里已经被证伪过两次
#: （回收三格读数、地表找星门）—— 正文行数一变，任何单一的框都会在某些信上系统性
#: 地读错。正确做法是出一串候选，让**外部事实**去裁；这里的外部事实就是容量不变量
#: （`回收船数 × 单船容量 ≈ 金+晶+气`），和挑三格读数用的是同一条。
#:
#: 档位是量出来的，不是猜的（2026-09-13 夜，42 张现场图）：那一行的纵向位置
#: **跨度约 52px**（client y 408..460，约两个正文行高）——正文两行版、三行版、
#: 外加「舰队返回」那种多行编成，各自把下面整块顶到不同高度。标定框只有 28px 高，
#: 单靠它只能接住其中一档。
#:
#: ⚠️⚠️ **试过铺到 +40（八档），一封都没多读通** —— 同一批语料上
#: 八档读通 8/22、四档读通 8/23。所以后面那四档**没留**：每多一档就是多给
#: 「框住隔壁那一行、读出一个同样自洽的数」发一张牌，挣不到位置就是纯风险。
#: 下一步不该是继续加档，而是**锚定**（先找到「回收船」那三个字再读它右边）。
#:
#: ⚠️ **别再往里加档位来「提高命中率」。** 档位越多，越可能有某一档正好框住
#: **隔壁那一行**的数字，而那个数字同样是整数、同样能进容量判据 —— 多出来的候选
#: 是在给「读错但自洽」这件事发牌。
RECYCLE_SHIP_OFFSETS: tuple[int, ...] = (0, -8, 8, -16)
...
class ReportLayout:
    ...
    #: 「回收船」那一格。⚠️ **它只是候选里的第 0 档，不是唯一那一档** ——
    #: 详情页正文的**行数会变**（坐标与名字长了就折行），整块跟着上下浮动约 ±8px，
    #: 而这个框只有 28px 高，吃不住。实测 2026-09-13 夜：数字实际落在
    #: client y 408..419，而这个框换算过去是 412..440 —— 11px 高的数字被切掉 4px。
    #: 偏移档位在 `RECYCLE_SHIP_OFFSETS`，谁对由容量不变量裁。
...
LIVE_LAYOUT = ReportLayout(
    ...
    # ⚠️⚠️ **这个框一个像素都别动。** 2026-09-13 夜试过把右沿放宽到 1240
    # （理由看着很对：两位数会右移到 x 1200..1220，个位被切在框外，`74` 读成 `7`），
    # 结果是**语料库里原本 8/8 的那批当场挂了一封** —— 放宽改的是 offset-0 那一档，
    # 也就是原先唯一读得好好的那条路，而多进来的像素让它读出了别的数。
    # 收益那一侧同时量过：放不放宽，「原本读不出」那批都是读通 8 封，一封不多。
    # 版面的浮动交给 `RECYCLE_SHIP_OFFSETS` 去接，不要动基线。
    recycle_ship_count=Region(1140, 374, 1210, 402),  # 见上：这是第 0 档
