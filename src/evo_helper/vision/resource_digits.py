...
战报（408 格逐格人工核过真值；语料只放在本机 `var/` 下，**不进仓库**，理由见
`tests/integration/vision/test_resource_grid_corpus_live.py`）量下来，tesseract
在这个尺寸上**既读不全、又读得不对**：
...
- 孤零零一个 `0` 是最难的一格：任何倍数、任何 psm、任何预处理下，tesseract 给的
  都是 `''` / `0` / `5` / `2` 之间的一次抛硬币。而好几格常年就是一个 `0`。

⚠️ **代价不是「少一份数据」，是「多一份假数据」。** 生产库里那 5 份存下了收获的
战报，逐格核对之后有 **2 份的数量是错的**——入库之后没有任何办法回头分辨，
因为一个读错的数量和真的数量长得一模一样，既不越界、也不违反任何格式。
...
3. **动态规划切字**：`best[x]` 是「把前 x 列解释完」的最优对数似然，转移是「在这里
   放某个字模」或「跨过一列全空的缝」。**不预先按空列切段**——相邻两个 `1` 之间
   没有空列，按段切会把它们并成一个字形（实测 34 份里就有一份栽在这上面）。
...
⚠️ **剩下 5 格仍然读错**，全部落在 `3`→`9`（两处）、`5`→`9`、`6`→`8`、`8`→`9`
这几对形近字上（逐份的清单只存在本机的真值表里，仓库里不记具体数量）。408 格里错 5 格
...
#: 9 列宽的 `K` 就没地方落脚，DP 会拿 7 列宽的数字去顶——实测好几份的**末尾那个
#: `K` 被读成了一个数字**，于是量级凭空掉了三个数量级，而读出来的串照样是个
#: 合法数量。留两列之后这一类错误全部消失。
BAND_PAD = 2
...
def decode_band(strip: list[list[float]]) -> tuple[str, float]:
    ...
    ⚠️ **不按空列预先切段。** 相邻两个 `1` 之间没有背景列，切段会把它们并成一个
    字形，接着被当成某个 7 像素宽的数字——其余各位全对、只是少了一位，
