...
def pick_count(votes: Mapping[str, int]) -> str:
    ...
    （bot 一律 BBB），但这一笔仍然要紧：守方「单位」直接进情报中心，
    差 100 倍就是页面上凭空多出 12 万艘船；它还是**兜底算式的四个输入之一**
    （剩余 = 单位 − 损失，`domain.battle_outcome`）——2026-08-17 之后战果以
    横幅为准，这套算式只在横幅读不出来时才顶上，但顶上的那几份照样会被它记错。
