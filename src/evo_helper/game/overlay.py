...
#:
#: ⚠️ **这是第二道独立的闸，不是 IoU 的装饰。** 它挡的是「一大片白正好盖住这一块」：
#: 一屏全白时（浏览器还在加载，`var/logs/rankv/00-baseline.png`）点阵框里「全中」，
#: IoU 恰好等于 167/306 = 0.546——**离阈值 0.60 只剩 0.054**。今天 IoU 一条确实
#: 也拦得住它，但拦得只有这么一点富余，而另一侧真正的 ✕ 最低是 0.873：
#: 单看 IoU 的话，阈值往下挪一丁点就会把「一片白」认成关闭键。
#:
#: 静默环把这道界拉开成无争议的：✕ 是**孤立**的图形，实拍上认出来的 92 张这一圈
#: 的白占比没有超过 0.012 的，而整屏泛白那张是 1.000。两条判据看的是不同的东西
#: （形状 / 孤立性），所以**必须同时成立**（`CloseButtonLook.visible`）。
