...
⚠️ **`blind_scrolls`（屏）那一列保留不删，这条迁移一个字都不碰它。** 它是这次改动的
回滚落脚点：命令行上只给 `--blind-scrolls` 不给 `--blind-rows` 就退回慢拖那条老路，
**不必再来一条迁移**。顺手清理掉它，回滚就变成「改代码 + 加迁移 + 重新发版」。

⚠️ **回滚是命令行层的事，不是本列置空。** 本列置空的含义只有一个——上面那句「跟着
代码默认值走」，走的**仍是滚轮**（`storage.models.MilitaryAttackConfigRow
.blind_scroll_rows` 上记着这条口径：留空只能有一个含义，与本表其余可空旋钮一致）。
