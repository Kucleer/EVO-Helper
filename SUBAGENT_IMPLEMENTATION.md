...
| 阻塞 | 影响 | 需要的决定 |
|---|---|---|
| 浏览器控制通道未能落盘全分辨率截图（`save_to_disk` 只成功一次，且输出 754×355 缩略图） | 无法为 CP5 建立视觉基线；本批次只有字段结构记录，没有图片工件 | 需要用户确认取图方式：手工截图放入 `var/captures/`，或授权其他采集通道 |
| 合并 PR #35 / #36 被权限分类器拦截 | `main` 仍停在 `ee1aebd`，两条已过 CI 的分支未入主干 | 需要用户放行合并，或由用户自行合并 |
...
1. 解除上表两项阻塞。
2. 取到攻击报告 / 战斗回放的全分辨率图片后，补齐 `artifact_id`、UTC 时间、会话、批次、viewport、SHA-256、来源，并更新 `docs/ui-version-matrix.md`。
3. 以本节字段结构实现并验证 GamePort 的新邮件导航、攻击报告详情与回放解析；未知 UI 一律 fail-closed。
