...
> **续办入口已移至 [HANDOFF.md](HANDOFF.md)**（2026-08-08）。
> 那里记录了环境前提、实测的界面事实、OCR 配方和踩过的坑。请先读它，再看本节的历史记录。
...
前端已从朴素表格后台重构为深色作战台，主导航收敛为**任务中心**与**情报中心**，
运行详情与诊断作为辅助页。技术栈仍是 Jinja2 + 原生 JS，没有引入独立前端构建链。

| 路径 | 内容 |
|---|---|
| `/missions` | 统计瓦片、多区段任务表单（预填 `探路` 预设）、历史任务列表 |
| `/intel` | 顶部紧凑筛选条 + 宽结果列表 + 历史战报弹窗 |
| `/runs`、`/runs/{id}` | 启动运行、状态徽标、暂停/恢复/紧急停止 |
| `/diagnostics` | 强制复查、复查列表、状态事件时间线 |
| `/`、`/plans` → `/missions`；`/targets` → `/intel` | 旧路径重定向，不留死链 |

设计约束：

- **颜色从不是唯一信号**：每个状态徽标同时给出颜色、字形与状态名，灰度与色盲下均可读。
- **`dry run` 只展示不提供开关**。真实派遣仍只能走 Canary 流程，界面上没有任何入口。
- 列表只显示 深空吞噬者 / 噬能截击者 / 钛能守卫者 / 收割者 四列，其余归入「其他舰种」并在详情展开；
  每种舰种一列会宽过任何笔记本屏幕。缺失显示 0，数值列居中且列名对齐，滚动时表头锁定。

配套的检索能力：`POST /api/intel/search` 支持坐标范围 + 条件树（总数/单个舰种，AND/OR 嵌套）
+ 分页排序，全部服务端过滤；`/api/intel/filters` 持久化命名筛选器（迁移 `b7c2d1e40a55`）。
条件树**没有**编译成 SQL——候选集已被坐标区间收敛，复用领域求值器可保证 AND/OR 语义只有一份实现。

### 25.13 期间修正的既有缺陷

- **出发星球不再要求落在扫描区间内**。它是玩家自己的星球，大概率不在被扫描区间内；
  该校验在 Fake 与持久化两个服务里各有一份，只改一处会让控制台继续报错。
- **舰种名称 OCR 吸附**：名称是舰队时间线做差异的键，错乱会让每份报告都显示成「首次出现」。
- **舰种目录按游戏内清单重建**（`SHIP_ORDER` 18 + `DEFENCE_ORDER` 11），
  纠正了 `运输舰`→`小型运输船`、`间谍探测器`→`探测器`，补入 `收割者`、`湮灭之星`、导弹与护盾。

### 25.14 下一步（按顺序）

采集链路是唯一剩余的阻塞项，需要用户在场：
...
`探路` 预设（`轻型战斗机:1`）已配置为默认值。**签名取舰种组成而非名称**：
`探路` 只有两个字，低于 OCR 吸附阈值 `MIN_SNAP_LENGTH = 3`，名称读错无法靠词表修复，
组成签名才是安全不变量 9 里可靠的那一半。
