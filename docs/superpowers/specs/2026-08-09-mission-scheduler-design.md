...
| 什么时候能再派？ | 出发 + 飞行时长 **× 2**（舰队要飞回来才释放航线） | `line_free_at_utc` |
...
## 十三、两个阻断弹窗（用户 2026-08-10 补充，**ROI 待实机标定**）

派遣链路上有两个会挡住整轮的游戏弹窗。两个都不是故障，是正常的资源耗尽，
...
- **不要重试、不要点别处。** 认出来就关掉弹窗、结束这一轮、正常退出（退出码 0）。
  这不是失败，不该计入连续失败计数，否则连撞三次就会把整条链路自动停用。
- **把「下次什么时候值得再来」写回库**，让调度器据此安排，而不是靠冷却硬等。
- 弹窗的 ROI 与关键词**需要一次实机标定**（用户已提供截图，坐标要在 1920×917
  的 client 空间重新量）。在标定完成前，这两个画面对 runner 是不可见的——
  它会走到「认不出的画面一律停止」那条既有护栏上，安全但不优雅。
