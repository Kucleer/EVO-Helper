...
**计数口径（用户 2026-08-10 修订）：当日海盗战报数与当日海盗派遣数取大者。**

- 战报数（`battle_reports ⋈ dispatch ⋈ intent`，`target_kind=PIRATE`，当日 UTC）
  能算上**用户手动打的那几发**——这正是改用它的理由。
- 但战报是抵达后才产生的，**只数战报会漏掉在飞的**：连派 5 发、一份战报都没回，
  计数仍是 0，于是继续派——恰好造成要防的那件事。所以派遣数必须并入。
- 取大者只会比两者中任何一个更保守。宁可少打一次，也不白飞一趟舰队。
