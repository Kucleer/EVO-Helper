...
⚠️ **硬信号这条目前是死路**：调度器已经正确读这一列，但**全仓没有任何代码写它**——
认出「超限通知」那封邮件需要一个谁都还没见过的画面的 ROI 与关键词，没有实机截图
就没法定。所以当前生效的只有计数那一条判据。这不阻塞上线（计数本身是准的），
但要记住：如果用户手动打了几发海盗，计数会偏低，而唯一能纠正它的那条信号是关的。
