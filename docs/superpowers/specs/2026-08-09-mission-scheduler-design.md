...
日期：2026-08-09
需求来源：交接文档《TODO-交接.md（已于 2026-08-21 删除，见 AGENTS.md 0.2；内容在 git 历史里）》需求 4「控制台整合」，经本轮澄清后大幅扩写。
...
> ⚠️ **本节已作废（2026-08-17）。** 桌面悬浮窗 `tools/scan_console.py` 连同它的
> 全局快捷键整个删掉了——控制台能远程访问之后，它显示的三样页面上全有（用户
> 口径「关闭桌面悬浮窗，因为现在可以远程控制了」）。下面留着是记录当时的判断，
> 不是现状。本节里唯一还活着的是「一个鼠标只许一个主人」那条不变量，它现在由
> `application/mission_supervisor.MissionSupervisor` 独占进程启动来保证。
